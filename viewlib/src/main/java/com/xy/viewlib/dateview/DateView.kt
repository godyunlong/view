...
class DateView(context: Context, attrs: AttributeSet?=null):FrameLayout(context,attrs), DateClickListener,View.OnClickListener{
    ...
    init {
        ...
        val selectBackground = array.getResourceId(R.styleable.DateView_date_select_background,R.drawable.bg_black_corners_30)
        ...
        val selectTextColor = array.getColor(R.styleable.DateView_date_select_textColor,getColor(context,R.color.white))
        val selectTextColorNot = array.getColor(R.styleable.DateView_date_select_textColor_not,getColor(context,R.color.gray_9999))
        val toDayTextColor = array.getColor(R.styleable.DateView_date_toDay_textColor,getColor(context,R.color.red_e32c))
        val commonTextColor = array.getColor(R.styleable.DateView_date_common_textColor,getColor(context,R.color.gray_3333))

        adapter = DateAdapter(selectBackground,selectBackgroundNot,toDayBackGround,selectTextColor,selectTextColorNot,toDayTextColor,commonTextColor,this)
    ...
    override fun onDateClickListener(item: DateMode,position: Int) {
        if (item.nowMoth && adapter is DateAdapter){
            adapter?.changeSelectPosition(position)
        }else if (!item.nowMoth){
            jumpMonth(year, month,item.day, item.day>position)
        }
    }
    ...
    fun getSelectDate(): DateMode?{
        ...
        if (adapter.selectPosition in adapter.data.indices)
            return adapter.data[adapter.selectPosition]
        return null
    }
